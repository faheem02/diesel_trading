    ...
    if ($tank_id <= 0 || empty($adjustment_type) || $quantity <= 0) {
        ...
        if (!$tank) {
            ...
            try {
                ...
                $desc = "$adjustment_type - " . ($description ?: "No details");
                ...
                $conn->commit();
                $success = "Stock adjustment recorded successfully! Type: $adjustment_type, Qty: " . number_format($quantity, 3) . " tons.";
                $_POST = [];
            } catch (Exception $e) {
                $conn->rollback();
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="adjustment_type" class="form-control" required>
                            ...
                            <option value="Shortage" <?= (isset($_POST['adjustment_type']) && $_POST['adjustment_type'] === 'Shortage') ? 'selected' : '' ?>>Shortage</option>
                            <option value="Leakage" <?= (isset($_POST['adjustment_type']) && $_POST['adjustment_type'] === 'Leakage') ? 'selected' : '' ?>>Leakage</option>
                            <option value="Measurement_Difference" <?= (isset($_POST['adjustment_type']) && $_POST['adjustment_type'] === 'Measurement_Difference') ? 'selected' : '' ?>>Measurement Difference</option>
                        </select>
                    </div>
                </div>
            </div>
