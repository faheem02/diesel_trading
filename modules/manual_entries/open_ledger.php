<?php
session_start();
$active_page = 'manual_entry_ledger';
...
$names = $conn->query("SELECT DISTINCT person_name FROM manual_entries ORDER BY person_name ASC");

$entries = [];
$total_amount_sum = 0;
$paid_amount_sum  = 0;

if ($show && !empty($person)) {
    $stmt = $conn->prepare("SELECT * FROM manual_entries WHERE person_name = ? ORDER BY entry_date ASC, id ASC");
...
if ($print_mode && $show) {
    ...
    <div class="print-wrapper">
        ...
        <div class="subtitle">کل اندراجات: <?= count($entries) ?></div>
        <table>
            <thead><tr>
                <th>SR نمبر</th><th>تاریخ</th><th>تفصیل</th>
                <th>تعداد</th><th>فی دانہ</th>
                <th>کل رقم</th><th>وصولی</th><th>زریعہ وصولی</th><th>باقی</th>
            </tr></thead>
            <tbody>
                ...
                <tr class="total-row">
                    <td colspan="3">کل</td>
                    <td><?= number_format(array_sum(array_column($entries, 'quantity')), 3) ?></td>
                    <td></td>
        ...
        <div class="no-print" style="text-align:center;margin-top:20px;">
            ...
            <a href="open_ledger.php" class="btn-back">واپس</a>
        </div>
    </div>
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-book mr-1"></i> اندراجات Ledger</h1>
    <?php if ($show): ?>
    <div>
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-search mr-1"></i> نام منتخب کریں</h6>
    </div>
    <div class="card-body">
        <form method="GET" class="form-inline">
            <div class="form-group mb-2">
                <label class="small font-weight-bold mr-2">نام</label>
                <select name="person" class="form-control form-control-sm" id="personSelect" required>
                    <option value="">-- نام منتخب کریں --</option>
                    <?php while ($n = $names->fetch_assoc()): ?>
                        <option value="<?= htmlspecialchars($n['person_name']) ?>" <?= $person === $n['person_name'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($n['person_name']) ?>
                        </option>
                    <?php endwhile; ?>
                </select>
                <button type="submit" class="btn btn-sm btn-primary mr-2 ml-2"><i class="fas fa-eye fa-sm"></i> دیکھیں</button>
            </div>
        </form>
    </div>
</div>
            ...
            <table class="table table-bordered table-hover" id="ledgerTable" width="100%" cellspacing="0" style="direction:rtl; text-align:right;">
                ...
                <tfoot>
                    <tr class="font-weight-bold" style="background:#f8f9fc;">
                        <td colspan="3">کل</td>
                        <td class="text-right"><?= number_format(array_sum(array_column($entries, 'quantity')), 3) ?></td>
...
function printLedger() {
    window.open('open_ledger.php?person=<?= urlencode($person) ?>&print=1', '_blank', 'width=1100,height=700');
}
</script>
<?php endif; ?>
