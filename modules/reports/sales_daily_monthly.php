<?php
session_start();
$active_page = 'rpt_sales_daily_monthly';
...
if ($view === 'daily') {
    $sql = "SELECT sale_date AS label, COUNT(*) AS invoices, SUM(quantity) AS total_qty, SUM(total_amount) AS total_amount
            FROM customer_sales WHERE sale_date BETWEEN ? AND ?
            GROUP BY sale_date ORDER BY sale_date DESC";
} else {
    $sql = "SELECT DATE_FORMAT(sale_date, '%Y-%m') AS label, COUNT(*) AS invoices, SUM(quantity) AS total_qty, SUM(total_amount) AS total_amount
            FROM customer_sales WHERE sale_date BETWEEN ? AND ?
            GROUP BY DATE_FORMAT(sale_date, '%Y-%m') ORDER BY label DESC";
}
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-calendar-alt mr-1"></i> Sales Report — Daily / Monthly</h1>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <form method="GET" class="form-inline flex-wrap">
            ...
            <a href="sales_daily_monthly.php" class="btn btn-sm btn-secondary mb-2"><i class="fas fa-redo fa-sm"></i> Reset</a>
        </form>
    </div>
</div>
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="rptTable">
                ...
                <tbody>
                    <?php if (empty($rows)): ?>
                        <tr><td colspan="4" class="text-center text-muted py-4">No sales found.</td></tr>
